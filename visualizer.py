...
class Visualizer:
    ...
    def plot_backtest_results_by_periods(self, df, result, base_dir='./images',
        ...
        try:
            ...
            # 1. 과거 1년 데이터
            year_start = last_date - pd.Timedelta(days=365)
            year_df, year_result = self._filter_data_by_period(df, result, start_date=year_start, end_date=last_date)
            if len(year_df) > 0:
                save_path = os.path.join(base_dir, 'backtest_result_year.png')
                self.plot_backtest_results(year_df, year_result, save_path=save_path,
                                          buy_threshold=buy_threshold, sell_threshold=sell_threshold)
            
            # 2. 과거 1개월 데이터
            month_start = last_date - pd.Timedelta(days=30)
            month_df, month_result = self._filter_data_by_period(df, result, start_date=month_start, end_date=last_date)
            if len(month_df) > 0:
                save_path = os.path.join(base_dir, 'backtest_result_month.png')
                self.plot_backtest_results(month_df, month_result, save_path=save_path,
                                         buy_threshold=buy_threshold, sell_threshold=sell_threshold)
            
            # 3. 올해 데이터 (1월 1일부터)
            this_year_start = pd.Timestamp(current_date.year, 1, 1)
            this_year_df, this_year_result = self._filter_data_by_period(df, result, start_date=this_year_start, end_date=last_date)
            if len(this_year_df) > 0:
                save_path = os.path.join(base_dir, 'backtest_result_this_year.png')
                self.plot_backtest_results(this_year_df, this_year_result, save_path=save_path,
                                         buy_threshold=buy_threshold, sell_threshold=sell_threshold)
            
            # 4. 이번달 데이터 (이번달 1일부터)
            this_month_start = pd.Timestamp(current_date.year, current_date.month, 1)
            this_month_df, this_month_result = self._filter_data_by_period(df, result, start_date=this_month_start, end_date=last_date)
            if len(this_month_df) > 0:
                save_path = os.path.join(base_dir, 'backtest_result_this_month.png')
                self.plot_backtest_results(this_month_df, this_month_result, save_path=save_path,
                                         buy_threshold=buy_threshold, sell_threshold=sell_threshold)
            
            # 5. 과거 100일 데이터
            days_100_start = last_date - pd.Timedelta(days=100)
            days_100_df, days_100_result = self._filter_data_by_period(df, result, start_date=days_100_start, end_date=last_date)
            if len(days_100_df) > 0:
                save_path = os.path.join(base_dir, 'backtest_result_100.png')
                self.plot_backtest_results(days_100_df, days_100_result, save_path=save_path,
                                         buy_threshold=buy_threshold, sell_threshold=sell_threshold)
